...
创建一个类，叫Address，在这个类中，定义表中的相关字段。字段名采用驼峰命名方式。
...
2 把sql语句在DistrictMapper接口中定义出来，叫findNameByCode
...
2 在addNewAddress方法中奖districtService接口中获取到的省市区数据转移到address对象，这个对象中就包含了所有该插入的数据。
...
1 给设置默认收货地址按钮添加一个onclick属性，这个属性指向一个方法的调用；在这个方法中，完成ajax请求。

用户在address.html页面点击“设置默认”按钮，发送ajax请求。

2 在html中写方法。

```html
function showAddressList() {
            // 重新提醒一下：之所以默认的记录在最前面，是因为sql语句中已经排过序了
            $.ajax({
                url: "/addresses",
                type: "POST",
                dataType: "JSON",
                    ...
                    if (json.state == 200) {
                        ...
                        for (let i = 0; i < list.length; i++) {
                            ...
                            let tr = '<tr>\n' +
                                        ...
                                        '<td><a onclick="setDefault(#{aid})" class="btn btn-xs add-def btn-default">#{isDefault}</a></td>\n' +
                                     '</tr>';
                            // replace方法，有两种写法
                            ...
                            if (list[i].isDefault == 1) { // 数据库中读取为默认
                                tr = tr.replace("#{isDefault}", "");
                            }
                            else {
                                tr = tr.replace("#{isDefault}", "设为默认");
                            }
                            // <tbody>的id=address-list。往标签下追加内容。
                ...
                error: function (xhr) {
                    alert("调取收货地址时产生未知的异常。错误信息：" + xhr.message);
                }
            });
        }
        ...
        function setDefault(aid) {
            $.ajax({
                ...
                error: function (xhr) {
                    alert("设置默认收货地址时产生未知的异常。错误信息：" + xhr.message);
                }
            });
        }
```
...
1 删除之前，判断数据是否存在（已开发过）；判断该条地址数据的归属是否是当前用户（已开发过）？

不用重复开发

2 执行删除收货地址的信息d * from t_address where aid=?

```sql
delete * from t_address where aid=?
```

3 如果用户删除的是默认收货地址，将剩下地址中的某一条设置为默认的收货地址。规则可以自定义：例如修改时间最近的设置为默认收货地址（modified_time）。
...
4 如果用户本身就只有一条收货地址的数据，删除后，其他的操作就可以不再进行。
...
<delete id="deleteByAid">
    ...
    <select id="findLastModified" resultMap="AddressEntityMap" resultType="com.cy.store.entity.Address">
        SELECT * FROM t_address
        WHERE uid=#{uid}
        ORDER BY modified_time DESC LIMIT 0,1
    </select>
...
查询语句一般不会有异常，就算没查到也会是空值返回，不能认为是异常。

在执行删除时，可能会产生未知的删除异常，导致数据不能删除成功，此时抛出DeleteException异常（需要定义）。
...
将DeleteException统一纳入BaseController中。
...
借用showAddressList方法，在读取列表的时候，把aid的值绑定在onclick上。
...
注意：为tr替换内容的时候，对于出现多次的字符串，需要用replaceAll，因为replace在不使用全局搜索标志“g”结尾的情况时，只替换第一个匹配项。
...
在com.cy.store.mapper包下，创建ProductMapper接口，并在接口中添加查询热销商品findHotList()的方法。
...
<!-- namespace属性：
用于指定当前的映射文件和哪个接口进行映射，
所以需要指定接口的文件路径，故需标注包的完整结构-->
...
抽象方法的实现：在ProductServiceImpl中实现。
...
> 
> 是否拦截：否。需要将index.html和products/**请求添加到白名单。
...
public class ProductController extends BaseController{
    @Autowired
    private IProductService productServiceService;
    ...
    public JsonResult<List<Product>> getHotList() {
        List<Product> data = productServiceService.findHotList();
...
localhost:8080/products/hot_list

## 12.4 前端

1 在（web下的）index.html页面给“热销排行”列表的div标签设置id属性值。

```html
<div id="hot-list" class="panel-body panel-item">
```

2 在index.html页面中body标签内部的最后
...
增加一个ProductNotFoundException.java异常。

js的html方法，将json.data中的某个属性值填入指定标签，成为该标签的内容。html方法可以替换某个标签内部的所有内容，甚至添加形如<p></p>的内容后，p标签即生效。

attr方法，用于动态给名称有规律的标签内的**某个属性**修改内容。

相较于一大段标签改内容，直接改标签内的内容更好。
...
2 如果商品已经被加入过购物车，不需插入，而是更新数量

```sql
update t_cart set num = ?
where cid = ?
```

3 上面1和2的运行判断，依赖于查询是否存在相应记录。有记录了就更新数量，没有记录就插入。

```sql
select * from t_cart
where uid = ? and pid = ?
<!--之所以不查询唯一的cid，是因为在前端，cid不好获取-->
```
...
```xml
    ...
    <select id="findByCidAndPid" resultMap="UserEntityMap" resultType="com.cy.store.entity.Cart">
        SELECT * FROM t_cart
...
1 插入时可能产生异常：InsertException（已创建）

2 更新时可能产生异常：UpdateException
...
注意：标签取值是个方法，不是属性$("#标签id").val()

**重要回顾：在ajax函数中，data参数的数据设置方式：**

- data: $("#表单名") .serialize() 表单选择：参数过多且在同一个表单中，方便字符串的提交。

- data: new FormData($("#表单名")[0]) 只适用于提交文件

- data: "属性1=" + 值1 + "属性2=" + 值2 串的拼接，适合参数值固定，并且参数值列表有限，可以进行手动拼接
...
//启动对（加入购物车）按钮的监听
            $("#btn-add-to-cart").click(function () {
                $.ajax({
                    ...
                    data: {
                        "pid" : id, // script开头获取过了
                        "amount" : $("#num").val(), // 用选择器选中名为num的标签，获取它的值
                    },
                    dataType: "JSON",
                    success: function (json) {
                        if (json.state == 200) {
                    ...
                    error: function (xhr) {
                        alert("加入购物车时产生未知的异常。错误信息：" + xhr.message);
                    }
                });
            });
```
...
VO：Value Object，值对象。当进行SELECT查询时，查询结果属于多张表中内容的情况下，发现结果集不能直接使用某个POJO实体类接收（POJO实体类不能包含多表查询出来的结果）。

解决方式是：重新去构建一个新的对象，用于存储查询出来的结果对应的映射。称这种对象为值对象。

不建议放在entity包下，而是重新建立一个vo包并存放。

CartVO类也不需要继承，只需实现Serializable。写好变量后，实现getter()、setter()、 equals()、hashcode()、toString()方法。
...
1 代码原文中引入了carts.js，因为是写死的内容，需要注释掉。

2 了解form表单的结构。action="orderConfirm.html" tbody标签的id="cart-list" 把结算的type从submit改为button

3 编写json脚本。借助ready()函数，完成自动ajax请求的提交和处理。

复制大段代码小妙招：原样复制，新建一个变量，let tr=''，把复制到的内容粘贴在单引号内部。

别忘了给复选框赋一个cid值，这样方便计算价格
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        for (let i=0; i<list.length; i++) {
                            let tr = '<tr>\n' +
                                    '<td>\n' +
                                    '<input name="cids" type="checkbox" value="#{cid}" class="ckitem" />\n' +
                                    '</td>\n' +
                                    ...
                                    '<input id="price+#{cid}" type="button" value="+" class="num-btn" onclick="addNum(2)" />\n' +
                                    '</td>\n' +
                    ...
                    error: function (xrl) {
                        alert("购物车列表数据加载产生未知的异常！" + xrl.status);
                    }
                });
            }
...
1 执行更新t_cart表的记录的num的值，已存在，无需重复开发。

```sql
update t_cart
set num=?, modified_user=?, modified_time=?
where cid=#{cid}
```

2 根据cid的值，查询当前购物车的这条数据是否存在
...
2 查询的数据是否有访问权限（AccessDeniedException）

3 查询的数据不存在，抛出新的异常：CartNotFoundException
...
推荐用函数绑定的方法（找到tr字符串的onclick方法：onclick="addNum(#{cid})"，把addNum的参数值改为可替换的cid，加载时可以自动替换）。

需要定义addNum方法。

```js
'<input id="price+#{cid}" type="button"
value="+" class="num-btn"
onclick="addNum(#{cid})"
```
...
function addNum(cid) {
                $.ajax({
                    ...
                    success: function (json) {
                        if (json.state == 200) {
                            // 因为字符串的替代（cid的加入）已经完成了，
                            // 所以一旦加载结束，增减按钮绑定的onclick函数中，
                            // addNum()方法内部的cid值也已经确定
                            // 只需要把相应商品goodsCount标签赋值就好了
                            $("#goodsCount" + cid).val(json.data);
                            // 改总额（重新乘）
                            // 因为price的值不是放在value上，也不是放在哪个属性上，
                            // 所以用html方法直接获取标签下的内容（不管具体是什么）
                            let price = $("#goodsPrice" + cid).html();
                            let totalPrice = price * json.data;
                            // 选中显示总价的元素，把新的值放在内部而非赋值给某个属性
                            $("#goodsCast" + cid).html(totalPrice);
                        }
                        else {
                            alert("增加购物车数据失败，具体信息：" + json.message);
                        }

                    },
                    error: function (xrl) {
                        alert("增加购物车商品数量时，产生未知的异常！" + xrl.status);
                    }
                });
            }
```
...
用户在购物车列表页中通过随机勾选相关的商品，在点击“结算”按钮后，跳转到结算页面，在这个页面中，需要展示用户在上个页面所勾选的购物车对应的数据。

也是列表的展示，而展示的内容，仍是来自于购物车的表。

需要用户在购物车页面，将勾选的cid传递到下一个结算页面。
...
```java
List<Cart> findVOByCid(Integer[] cids);
```

### 17.1.3 SQL映射

学习：Mybatis的<foreach>标签，为将一个数组（或List等集合类元素）的值传入sql语句提供了可能。
    ...
    public List<CartVO> getVOByCid(Integer uid, Integer[] cids) {
        ...
        while (it.hasNext()) {
            ...
            if (! cartVO.getUid().equals(uid)) {
                // 从集合中移除这个元素
                // list.remove(cartVO);
                it.remove(); // 迭代器的remove方法，线程安全，不会引发并发异常
            }
        }
        return list;
    }
...
type="submit"默认将表单中的数据发送到由action属性指定的url，自动提交表单。

type="button"是一个普通按钮，点击后不会触发表单提交，需要结合javascript定义具体行为，没有任何默认的行为，仅响应用户的点击。

1 将cart.html页面中的“结算”按钮属性更改为type="submit"，这样可以跳转到新的页面，并在新的页面自动加载

2 在orderConfirm.html页面中添加自动加载从上个页面中传递过来的cids数据（注意：直接传到物理地址后面了，需要手动获取到并赋值给ajax请求的data），再请求ajax，进行填充当前页面某个区域
...
<script type="text/javascript">
        ...
        function showCartList() {
            // 清空tbody中的静态代码
            $("#cart-list").empty();
            console.log(location.search.substring(0));
            console.log(location.search.substring(1));
            $.ajax({
                url: "/carts/list",
                type: "POST",
                // cart.html的结算按钮用了submit，已经自动把cid参数传到url里
                // 系统只传递有name属性的组件的value，复选框有name所以传递
                // 但有一个问题：传递是直接传到物理地址后面了，而非控制层的虚拟地址
                // 为了传到虚拟地址里，需要把data主动获取到并注入
...
1 收货地址存放在一个下拉列表（select组件）中，将查询到的当前登录用户的收货地址动态地加载到这个下拉列表中。

实际上，已经编写了根据用户uid查询用户收货地址数据的功能（AddressController的getByUid方法），只需调用。

2 OrderConfirm.html页面中，收货地址数据的显示需要自动进行加载。
...
3 声明showAddressList方法，在其中定义ajax请求。
...
function showAddressList() {
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json.state == 200) {
                        let list = json.data;
                        // 顺便添加一个value属性，以便往其他页面传递
                        for (let i=0; i<list.length; i++) {
                            let opt = '<option value="#{aid}">#{name}   #{tag}   #{provinceName}#{cityName}#{address}   #{phone}</option>';
                ...
                error: function (xrl) {
                    alert("订单确认页面内 收货地址数据加载产生未知的异常！" + xrl.status);
                }
            });
        }
```
...
增加复选框选中检测功能，避免无选中的跳转

```java
function checkBoxSelected() {
                // 阻止表单提交如果没有复选框被选中
                $("#myForm").on("submit", function (e) {
                    let isChecked = $("input[name='cids']:checked").length > 0;
                    // isChecked是个布尔变量
                    if (!isChecked) {
...
创建一个OrderMapper接口，接口中添加以上**两个**SQL对象的抽象方法（OrderItem的增加项也添加到OrderMapper接口里）。
...
1 在IAddressService接口中，定义根据收货地址的id获取收货地址对应的数据。
...
3 在service包下，创建一个订单的接口IOrderService，添加抽象方法，用于创建订单。
